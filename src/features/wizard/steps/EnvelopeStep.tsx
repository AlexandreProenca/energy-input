...
function LayerList({ kind, presetId }: { kind: AssemblyKind; presetId: string }) {
  ...
  const a = preset.assemblies[kind];
  const t = assemblyThermal(kind, a.layers, templates.materials);
  return (
    <div className="grid gap-4 sm:grid-cols-[160px_minmax(0,1fr)]">
      ...
      <div>
        ...
        <p className="mb-2 text-xs text-slate-500">Camadas de fora para dentro</p>
...
export function EnvelopeStep() {
  ...
  const [detail, setDetail] = useState<AssemblyKind>('wall');
  ...
  return (
    <div className="space-y-8">
      <div role="radiogroup" aria-label="Tipo de construção" className="grid gap-3 md:grid-cols-3">
        {templates.constructionPresets.map((p) => {
        ...
        <div className="mb-4 overflow-x-auto">
          <Segmented
            ...
            options={(Object.keys(ASSEMBLY_LABEL) as AssemblyKind[])
              ...
              .map((k) => ({ value: k, label: ASSEMBLY_LABEL[k] }))}
          />
        </div>
        <LayerList kind={detail} presetId={env.presetId} />
      </div>
    </div>
  );
}
